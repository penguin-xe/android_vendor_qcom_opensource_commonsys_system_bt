...
static bool hh_connection_allowed(const RawAddress& bda) {
  ...
  if (added_dev != nullptr && added_dev->reconnect_allowed) {
    VLOG(1) << "Connection allowed " << bda;
    return true;
  } else if (btif_hh_cb.pending_conn_address == bda) {
    VLOG(1) << "Device connection was pending for: " << bda
                 << ", status: " << btif_hh_cb.status;
    return true;
  }
...
static void btif_hh_upstreams_evt(uint16_t event, char* p_param) {
  ...
  switch (event) {
    ...
    case BTA_HH_OPEN_EVT:
      ...
      if (!hh_connection_allowed(p_data->conn.bda)) {
        LOG(WARNING) << "Reject unexpected incoming HID Connection, device: " << p_data->conn.bda;
...
static bt_status_t disconnect(RawAddress* bd_addr, UNUSED_ATTR bool reconnect_allowed) {
  ...
  if (!reconnect_allowed) {
    LOG(INFO) << "Incoming reconnections disabled for device " << *bd_addr;
    btif_hh_added_device_t* added_dev = btif_hh_find_added_dev(*bd_addr);
    if (added_dev != nullptr && added_dev->reconnect_allowed) {
